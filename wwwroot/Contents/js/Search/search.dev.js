var currentSellPageIndex = 0;
var currentBuyPageIndex = 0;
var currentCompanyPageIndex = 0;
...
function filterProduct(type) {
    query.PageIndex = 0;
    if (type === typeBuy) {
        query.SearchType = typeBuy;
    } else {
        query.SearchType = typeSell;
    }
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res == null || res.listProduct === '' || res.listProduct.trim() === '') {
                if (query.SearchType === typeBuy) {
                    ...
                    $('#total').text(genTextHaveAll(0, 'chào mua'));
                    totalProductBuy = 0;
                }
                    ...
                    $('#total').text(genTextHaveAll(0, 'chào bán'));
                    totalProductSell = 0;
                }
            } else {
                if (type == typeBuy) {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (res == null || res.listCompany === '' || res.listCompany.trim() === '') {
                ...
                handleReadMore("#readmore-company", res.left, true);
...
function viewMoreProduct(type) {
    if (type === typeBuy) {
        query.PageIndex = currentBuyPageIndex + 1;
        query.SearchType = 1;
        currentBuyPageIndex = query.PageIndex;
    } else {
        query.PageIndex = currentSellPageIndex + 1;
        query.SearchType = 2;
        currentSellPageIndex = query.PageIndex;
    }
    $.ajax({
        url: 'Search/GetListProduct',
        type: 'POST',
...
function viewMoreCompany() {
    query.PageIndex = currentCompanyPageIndex + 1;
    query.SearchType = 3;
    currentCompanyPageIndex = query.PageIndex;
    $.ajax({
        url: 'Search/GetListCompany',
        type: 'POST',
        data: query,
...
function handleReadMore(obj, left, isCompany = false) {
    if (left > 0) {
        $(obj).show();
        if (isCompany)
            $(obj + " span").text(`Xem thêm ${left} doanh nghiệp`);
        else $(obj + " span").text(`Xem thêm ${left} sản phẩm`);
